        ...
        if(empty($jw_id) || !isset($jw_id)){
            ...
            if($getJwInfo){
                ...
                if(strpos($jw_yccym,'qcloud.la') !== false){ 
                    ...
                    $longUrl = $jw_yccym . '/index.html?jwid=J' . $jw_id;//. '&token=' . $jw_token . '&t=' . $timeNum . 'f=douyin&by=yy';
                }elseif(strpos($jw_yccym,'volces.com') !== false){ 
                    
                    // 微信云托管//友源科技2024-01-02修改
                    $longUrl = $jw_yccym . '#jwid=J' . $jw_id;
                }else {
                    
                    // 其他//友源科技2024-01-02修改
                    $longUrl = $jw_yccym . '/?jwid=J' . $jw_id;// . '&token=' . $jw_token . '&t=' . $timeNum . 'f=douyin&by=ylb';
                }
                // 有结果
                $result = array(
        		    'code' => 200,
