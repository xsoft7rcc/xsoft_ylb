<?php
    ...
    if(isset($_SESSION["yinliubao"])){
        ...
        $ycc = getDomainsByType($db, 5, $user_group); // OSS云存储通用域名
        $ycl = getDomainsByType($db, 6, $user_group); // 阿里云草料专用域名
        $yjs = getDomainsByType($db, 7, $user_group); // 阿里云金山文档域名
        //$yhs = getDomainsByType($db, 8, $user_group); // 火山引擎域名
